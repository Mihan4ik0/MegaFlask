...
class SearchableMixin(object):  # класс для поддержки поиска
    @classmethod
    def search(cls, expression, page, per_page):  # заменяет список идентификаторов объектов на фактические объекты
        ids, total = query_index(cls.__tablename__, expression, page, per_page)  # возвращает список ID результатов и их общее количество
        if total == 0:
...
class Post(SearchableMixin, db.Model):  # таблица постов в БД
    __searchable__ = ['body']
    id = db.Column(db.Integer, primary_key=True)
