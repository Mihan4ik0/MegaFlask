...
@app.route('/index', methods=['GET', 'POST'])  # Отлавливание главной страницы
...
def index():  # логика главной страницы
    ...
    posts = current_user.followed_posts().paginate(page, app.config['POSTS_PER_PAGE'], False)  # отображение постов только от отслеживаемых
...
def explore():  # логика страницы с постами от всех пользователей
    ...
    posts = Post.query.order_by(Post.timestamp.desc()).paginate(page, app.config['POSTS_PER_PAGE'], False)  # логика отображение постов
